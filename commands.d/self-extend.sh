...
function selfExtend() {
  ...
  if [[ -d "${extensionDirectory}" ]]; then
    ...
    if ! interactive::confirm "Do you want to overwrite the existing ⌜${extensionName}⌝ extension?"; then
...
function selfExtend_downloadTarball() {
  ...
  progress::start template="<spinner> Download in progress, please wait..."
...
function selfExtend_getSha1() {
  ...
  if [[ ${repositoryUrl} =~ "https://github.com/"([^/]+)"/"([^/]+) ]]; then
    ...
    # get the sha1
    REPLY=""
    progress::start template="<spinner> Fetching reference information from GitHub..."
...
function selfExtend_executeSetupScript() {
  ...
  if ! interactive::confirm "Do you trust the setup script for the extension ⌜${extensionName}⌝ and wish to execute it?"; then
