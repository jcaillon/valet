...
function testProfiler() {
  ...
  ("${VALET_HOME}/valet" -x self mock2 arg1 arg2)
  ...
  echo "→ VALET_STARTUP_PROFILING=true valet --log-level error -x self mock1 logging-level"

  (VALET_STARTUP_PROFILING=true "${VALET_HOME}/valet" --log-level error -x self mock1 logging-level)
