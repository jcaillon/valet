...
function testSelfUpdateNothingToDo() {
  ...
  echo "→ selfUpdate --skip-extensions"
  selfUpdate --skip-extensions && exitCode=0 || exitCode=$?
...
#   echo "→ SelfUpdate --no-shim --no-path --no-examples --no-extras --skip-extensions"
#   SelfUpdate --no-shim --no-path --no-examples --no-extras --skip-extensions && exitCode=0 || exitCode=$?
