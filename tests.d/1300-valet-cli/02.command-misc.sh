...
function testParseAndCheckArgument() {
  ...
  test::endTest "Testing that we correctly parse arguments and options and fail if they don't match" 1
}
...
function testCommandWithSudo() {
  ...
  test::endTest "Testing that a command with sudo ask for sudo privileges" $?
}
